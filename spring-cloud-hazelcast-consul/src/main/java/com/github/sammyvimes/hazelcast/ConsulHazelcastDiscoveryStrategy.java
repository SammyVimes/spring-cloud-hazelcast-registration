    ...
    private ConsulRegistration createRegistration() {
        ...
        if (StringUtil.isNullOrEmpty(propertiesHost)) {
            ...
            if (!Objects.equals(propertiesHost, discoveryNodeAddress)) {
                getLogger().warning(String.format("Probable misconfiguration, configured hazelcast host [%s] and " +
                        "host advertised via Consul [%s] doesn't match. This can lead to an error.", discoveryNodeAddress, propertiesHost));
            }
            host = propertiesHost;
        }
...
//            } else {
            ...
            for (final ServiceInstance service : response) {
                ...
                getLogger().info("Discovered healthy node: " + discoveredAddress + ":" + service.getPort());
            }
//            }
    ...
    private void shutdownHook() {
        ...
        try {
            getLogger().info("Deregistering myself from Consul: " + this.hazelcastRegistration.getServiceId());
